...
$(document).ready(function () {
    ...
    $('body').on('click', '.toSubscribe', function () {
        $('html, body').animate({
    ...
    if ($('.flexslider').length) {
        $('.flexslider').flexslider({animation: "slide", animationLoop: false, useCSS: Modernizr.touch});
    }
    var get_date = $('#countdown').data('event-date');
    if (get_date) {
        $("#countdown").countdown({date: get_date, format: "on"});
    }
    ...
    //        infinite: false,
    //        arrows: true,
    //        responsive: [{breakpoint: 1200, settings: {arrows: true, slidesToShow: 5}}, {
    //            breakpoint: 992,
...
$.validator.setDefaults({
    ...
    }, errorElement: 'small', errorClass: 'help-block', errorPlacement: function (error, element) {
