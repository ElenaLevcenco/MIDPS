...
App.run(["$rootScope", "$state", "$stateParams",  '$window', '$templateCache', function ($rootScope, $state, $stateParams, $window, $templateCache) {
  ...
  $rootScope.app = {
    name: 'Official Alert',
    description: 'Panoul utilizatorului',
...
function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
  ...
  $stateProvider
          ...
          resolve: angular.extend(helper.resolveFor('oitozero.ngSweetAlert', 'fastclick', 'modernizr', 'icons', 'screenfull', 'animo', 'slimscroll', 'classyloader', 'toaster', 'whirl'),
              {
                  auth: function ($q, authenticationSvc) {
                      ...
                      if (userInfo && userInfo != null && userInfo != 0 || sessionStorage["userInfo"]) {
      ...
      .state('app.feeds', {
          ...
          resolve: helper.resolveFor('loaders.css', 'spinkit', 'oitozero.ngSweetAlert')
      })
    // Blog
    // ----------------------------------- 
    .state('app.blog', {
        ...
        resolve: helper.resolveFor('truncate','loaders.css', 'spinkit','akoenig.deckgrid','socialLinks')
    })
    //.state('app.blog-post', {
    //    url: '/post',
    //    title: 'Post',
    //    templateUrl: helper.basepath('blog-post.html'),
    //    resolve: helper.resolveFor('angular-jqcloud', 'socialLinks')
    //})
    .state('app.articles', {
        url: '/articles',
        title: 'Articles',
        templateUrl: helper.basepath('blog-articles.html'),
        resolve: helper.resolveFor('datatables','socialLinks')
    })
    .state('app.article-view', {
        ...
        templateUrl: helper.basepath('blog-post.html'),
        resolve: helper.resolveFor('ui.select', 'textAngular', 'socialLinks', 'truncate')
    })
    // 
    // Single Page Routes
    // ----------------------------------- 
    ...
    .state('page.register', {
        ...
        resolve: helper.resolveFor('oitozero.ngSweetAlert','loaders.css', 'spinkit')
    })
      .state('page.recover', {
          url: '/recover',
          title: "Register",
...
App
  ...
  .constant('APP_REQUIRES', {
    ...
    scripts: {
      'truncate':           ['vendor/truncate/truncate.js'],
      'socialLinks' :             ['vendor/share/angular-share.js'],
...
//Base defs
App.factory('apiBaseDef', ['$window', function ($window) {
    var session;
 ...
 * Demo for login api
 =========================================================*/
App.factory("authenticationSvc", ["$http", "$q", "$window", "apiBaseDef", 'Notify',
    function ($http, $q, $window, apiBaseDef, Notify) {
    var userInfo;
    function login(userName, password) {
        ...
        Notify.alert(
            '<div class="line-scale-pulse-out pulse-load">' +
            '<div></div>' +
            '<div></div>' +
            '<div></div>' +
            '<div></div>' +
        ...
        $http({
            url: apiBaseDef.url + '/dashboard/login',
            method: 'POST',
            data: $.param({email: userName, password: password}),
            headers: apiBaseDef.headers
        })
            .then(function (result) {
                $('.uk-notify-message').fadeOut(function() {
                    
                    
                    if (result.data.code != 0) {
                        
                        userInfo = {
                            userToken: result.data.data[0]
                        };
                        
                        
                        apiBaseDef.headers.userToken = userInfo.userToken.content;
...
App.controller("LoginFormController", ["$scope","$q","$http", "$location", "$window", "authenticationSvc",'apiBaseDef','Notify','$timeout',
    function ($scope,$q,$http, $location, $window, authenticationSvc,apiBaseDef,Notify,$timeout) {
        var userInfo = null;
        
        if ($window.sessionStorage.userInfo != 'null')
        {
            if (apiBaseDef.headers.userToken) {
            ...
            authenticationSvc.login($scope.userName, $scope.password)
                .then(function (result) {
                    ...
                    $scope.userInfo = result;
                        ...
                        if ($window.sessionStorage.blogUrl)
                        {
                            var url = $window.sessionStorage.blogUrl;
                            $window.sessionStorage.blogUrl = '';
                ...
                if (response.authResponse) {
                    FB.api('/me?fields=id,name,email', function (response) {
                       // console.log(response);
                        ...
                        $http({
                            url: apiBaseDef.url + '/dashboard/login/facebook',
                            method: "POST",
                            headers: {
                                'Content-Type': "application/x-www-form-urlencoded",
                                'Authorization': 'Basic b2ZmaWNpYWxAdGVzdC5jb206YXBpT3x8aWNpYWw='
                            },
                            data: $.param({id: response.id, email: response.email, name: response.name})
                        }).then(function (result) {
                            
                            
                            $window.sessionStorage["userInfo"] = JSON.stringify({ userToken: result.data.data[0]});
                            deferred.resolve({userToken: result.data.data[0]});
                            $('.uk-notify-message').fadeOut(function () {
                                if (result.data.code != 0) {
                               
                                    apiBaseDef.headers.userToken = result.data.data[0].content;
                                    apiBaseDef.headers.Authorization = 'Basic b2ZmaWNpYWxAdGVzdC5jb206YXBpT3x8aWNpYWw=';
//                                    $window.sessionStorage["userInfo"] = JSON.stringify({ userToken: result.data.data[0]});
                                    deferred.resolve({userToken: result.data.data[0]});
                                    $timeout(function () {
                                        if($window.sessionStorage.blogUrl)
                                        {
                                          $window.location.href = '/#'+$window.sessionStorage.blogUrl;
                                        }
                                        else
                                        {
                                          $location.path("app/dashboard");
        ...
        var deferred = $q.defer();
            ...
            $scope.result = authResult.getBasicProfile();
            

            $scope.response = {
                email: $scope.result.U3,
                name: $scope.result.ig,
                id: $scope.result.Eea
            }
            ...
            $http({
                ...
                data: $.param({id: $scope.response.id, email: $scope.response.email, name: $scope.response.name})
            }).then(function (result) {
...
App.controller('FeedsController', ['$scope', '$http', '$state','apiBaseDef', 'Notify',
    function($scope, $http, $state, apiBaseDef, Notify) {
        ...
        $scope.filterNational = {type: 1};
        $scope.filterLocal = {type: 2};
    ...
    //get feeds
    $http({
            url: apiBaseDef.url + '/dashboard/institutions',
            method: "GET",
            headers: apiBaseDef.headers
    }).then(function (result) {

        $('.loader-demo').fadeOut('slow');
        $scope.subscribeData = result.data.data;

        $scope.opts = {};
        angular.forEach($scope.subscribeData, function (data, index) {
            $scope.opts[data.id] = data.selected;
        });
    })
        ...
        $scope.checkAll = function () {
            ...
            angular.forEach($scope.subscribeData, function (item) {
                $scope.opts[item.id] = $scope.selectedAll;
            });
    ...
    $scope.saveFeeds = function () {
        ...
        angular.forEach($scope.opts, function (value, index) {
            if (value != 0) {
                $scope.institutions.push(index);
            }
        });
        var str1 = $scope.institutions.toString();
        var final = '[' + $scope.institutions + ']';
...
App.controller('RegisterController', ['$scope', '$http', '$state', '$cookies', 'apiBaseDef', '$timeout','Notify',
    function($scope, $http, $state, $cookies, apiBaseDef, $timeout, Notify) {

    //var regCookie = $cookies.PHPSESSID;
  ...
  $scope.register = function() {
    ...
    if($scope.registerForm.$valid) {
      ...
      $http({
            url: apiBaseDef.url + '/dashboard/register',
            method: "PUT",
            data: $.param({
                    ...
                    'verify-password':$scope.register.account_password_confirm
                }),
            headers: apiBaseDef.headers
        })
...
App.controller('RecoverController', ['$scope', '$http', '$state', '$cookies', 'apiBaseDef', '$state',
    function ($scope, $http, $state, $cookies, apiBaseDef, $state) {

        $scope.recover = function () {
            if ($scope.recoverForm.$valid) {
                $http({
                    url: apiBaseDef.url + '/dashboard/recovery',
                    method: "POST",
                    data: $.param({email: $scope.recover.email}),
                    headers: apiBaseDef.headers
                }).then(function (result) {
                    if(result.data.code != 1) {
                        swal({
...
App.controller('RecoverPassController', ['$scope', '$http', '$state', '$cookies', 'apiBaseDef', '$state','$window',
    function ($scope, $http, $state, $cookies, apiBaseDef, $state, $window) {

        var token = $cookies.email_token;
        //console.log(token)

        $scope.recoverPasssword = function () {
            if ($scope.recoverPass.$valid) {
                $http({
                    url: apiBaseDef.url + '/dashboard/recovery/confirm',
                    method: "PUT",
                    data: $.param({
                        email_token: token,
                        password: $scope.register.password,
                        verify_password: $scope.register.account_password_confirm
                    }),
                    headers: apiBaseDef.headers
                }).then(function (result) {
                    if(result.data.code != 1) {
...
App.controller('ProfileController', ['$scope', '$http', '$state', '$cookies', 'apiBaseDef', 'Notify',
    function($scope, $http, $state, $cookies, apiBaseDef, Notify) {

        $http({
            url: apiBaseDef.url + '/dashboard/profile',
            method: "GET",
            headers: apiBaseDef.headers
        }).then(function(result) {
        ...
        $scope.saveProfile = function() {
            ...
            if ($scope.data.first_name) {
                dataProfile['first_name'] = $scope.data.first_name;
            }

            if ($scope.data.surname) {
                dataProfile['surname'] = $scope.data.surname;
            }
            if ($scope.data.address) {
                dataProfile['address'] = $scope.data.address;
            }

            if ($scope.data.phone) {
                dataProfile['phone'] = $scope.data.phone;
            }

            //if ($scope.data.email_notify) {
            dataProfile['email_notify'] = $scope.data.email_notify;
            //}
        ...
        $scope.changePass = function() {
            ...
            if ($scope.data.current_password) {
                dataPassword['password'] = $scope.data.current_password;
            }

            if ($scope.data.new_password) {
                dataPassword['new_password'] = $scope.data.new_password;
            }

            if ($scope.data.new_pass_repeat) {
                dataPassword['verify_password'] = $scope.data.new_pass_repeat;
            }

            if (dataPassword && $scope.passwordEdit.$valid) {
                $http({
                    url: apiBaseDef.url + '/dashboard/password',
                    method: "PUT",
                    data: $.param(dataPassword),
                    headers: apiBaseDef.headers
                })
                    .then(function (result) {
                        $('.uk-notify-message').fadeOut(function () {
...
App.controller('BlogsController', ['$scope', '$http','apiBaseDef', "$filter",
    function ($scope, $http, apiBaseDef, $filter) {
        ...
        $http({
            url: apiBaseDef.url + '/dashboard/articles',
            method: "GET",
            headers: apiBaseDef.headers
        })
            .then(function(result) {
                ...
                angular.forEach(result.data.data, function(data,x) {
                    var time = new Date(data.published.replace(/-/g, '/'));
                    var t = $filter('date')(time, "yyyy/MM/dd HH:mm");
                    //console.log(data)
                    data.published = t;
                });
        ...
        $scope.show_article = function (e) {
            var id = $(e.target).data('id');
            $scope.article = $filter('filter')($scope.news, function (d) {
                return (d.article_id == id )
            })[0];

            $('.articles').animo({animation: 'fadeOutDown', duration: '0.2', keep: true}, function() {
                $('.articles').hide();
                $('.article').css('display','block');
                $('.article').animo({animation: 'fadeInDown', duration: '0.2', keep: false}, function() {
                    //$('.articles').hide();
                });
            });
        };

        $scope.show_articles = function () {
            $('.loader-demo').fadeIn();
            $('.article').animo({animation: 'fadeOutDown', duration: '0.2', keep: true}, function() {
                $('.article').hide();
                $('.articles').css('display','block');
                $('.articles').animo({animation: 'fadeInDown', duration: '0.2', keep: false}, function() {
                    setTimeout(function () {
                        $('.loader-demo').fadeOut();
                    }, 500);
                });
...
App.controller('ArticleController', ['$scope','$stateParams','$http','apiBaseDef','$filter',
    function ($scope, $stateParams, $http, apiBaseDef, $filter) {

    var id = $stateParams.id;

    $http({
        url: apiBaseDef.url + '/dashboard/article?article_id=' + id,
        method: "GET",
        headers: apiBaseDef.headers
    }).then(function(result) {
        ...
        $scope.article = result.data.data[0];
        var time = new Date($scope.article.published.replace(/-/g, '/'));
        var t = $filter('date')(time, "yyyy/MM/dd HH:mm");
        $scope.article.published = t;
